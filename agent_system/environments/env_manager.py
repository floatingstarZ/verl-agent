...
class GymCardEnvironmentManager(EnvironmentManagerBase):
    def __init__(self, envs, projection_f, config):
        self.text_only = bool(config.env.get("gym_cards", {}).get("text_only", False))
        super().__init__(envs, projection_f, config)
    
    def reset(self, kwargs) -> Dict[str, Any]:
        obs, infos = self.envs.reset()
        text_obs = self.build_text_obs(infos)
        observations = {
            'text': text_obs,
            'image': None if self.text_only else obs,
            'anchor': text_obs.copy() if self.text_only else obs.copy(),
        }
    ...
    def step(self, text_actions: List[str]):
        ...
        if self.text_only:
            next_observations['image'] = None
            next_observations['anchor'] = text_obs.copy()
        else:
    ...
    def build_text_obs(self, infos: Tuple[Dict]=None) -> List[str]:
        ...
        for i in range(len(infos)):
            if 'ezpoints' in self.config.env.env_name.lower():
                ...
                if self.text_only:
                    obs = GYM_CARDS_NUMBERLINE_TEXT_TEMPLATE.format(
                        current_number=infos[i].get('Current', 'unknown'),
                        target_number=infos[i].get('Target', 'unknown'),
                    )
                else:
