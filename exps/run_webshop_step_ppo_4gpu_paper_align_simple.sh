...
# H100 flash-attn override (sm_90). This mirrors the successful GIGPO runs.
...
echo "[ENV] Step-PPO full target confirmed: H100 (sm_90), GPUs=$VISIBLE_GPU_COUNT"
...
bash <(
  ...
  trainer.experiment_name=step_ppo_qwen2.5_1.5b_4gpu_paper_align_full_e250 \
  ...
  "$@" \
  2>&1 | tee "logs/webshop_step_ppo_qwen25_15b_4gpu_paper_align_simple_$(date +%Y%m%d_%H%M%S).log"
