...
def compute_eto_loss_rm_and_step_weights(data: DataProto, token_level_scores, beta, env_name):
    """Return trajectory-level ETO loss and per-step dL/d(step_q) weights.

    The returned weights allow a second forward pass to backpropagate the exact
    first-order gradient without keeping every microbatch graph alive at once.
    """
    ...
    traj_uids = data.non_tensor_batch['traj_uid']
    uid_batch = data.non_tensor_batch['uid']
    episode_rewards = data.batch['acc']
    active_masks = data.non_tensor_batch['active_masks']
    ...
    for i in range(len(traj_uids)):
        ...
        traj_uid = traj_uids[i]
        env_group = uid_batch[i]
        if traj_uid not in traj_step_values:
            traj_step_values[traj_uid] = []
            traj_step_indices[traj_uid] = []
            traj_success[traj_uid] = episode_rewards[i]
            traj_env_group[traj_uid] = env_group

        traj_step_values[traj_uid].append(step_q_values[i])
        traj_step_indices[traj_uid].append(i)

    traj_q_values = {
        traj_uid: torch.stack(values).mean()
        for traj_uid, values in traj_step_values.items()
    }

    env_groups = {}
    for traj_uid, env_group in traj_env_group.items():
        env_groups.setdefault(env_group, []).append(traj_uid)

    total_loss = step_q_values.new_tensor(0.0)
    total_pairs = 0
    traj_grads = {
        traj_uid: step_q_values.new_tensor(0.0)
        for traj_uid in traj_q_values
    }

    for _, traj_list in env_groups.items():
        if len(traj_list) < 2:
            continue
        ...
        for succ_traj in successful_trajs:
            for unsucc_traj in unsuccessful_trajs:
                diff = traj_q_values[succ_traj] - traj_q_values[unsucc_traj]
                total_loss = total_loss + torch.nn.functional.softplus(-diff)

                # d softplus(-diff) / d diff = sigmoid(diff) - 1.
                grad_diff = torch.sigmoid(diff) - 1.0
                traj_grads[succ_traj] = traj_grads[succ_traj] + grad_diff
                traj_grads[unsucc_traj] = traj_grads[unsucc_traj] - grad_diff
                total_pairs += 1

    step_weights = torch.zeros_like(step_q_values)
    if total_pairs == 0:
        print("Warning: No valid trajectory pairs found for DPO training")
        return total_loss, step_weights

    total_loss = total_loss / total_pairs
    for traj_uid, traj_grad in traj_grads.items():
        indices = traj_step_indices[traj_uid]
        if not indices:
            continue
        per_step_grad = traj_grad / (total_pairs * len(indices))
        for idx in indices:
            step_weights[idx] = step_weights[idx] + per_step_grad
