...
class AdvantageEstimator(str, Enum):
    ...
    GiGPO = 'gigpo'
    ISTAR_RLOO = "istar_rloo"
    ISTAR_GRPO = "istar_grpo"
    ISTAR_REINFORCE_PLUS_BASELINE = "istar_reinforce_plus_baseline"
    PROCESS_OUTCOME_COMBINED = "process_outcome_combined"
...
def apply_invalid_action_penalty(data: DataProto, invalid_action_penalty_coef=float):
    ...
    if 'step_rewards' in data.batch.keys():
        ...
        step_rewards = torch.sum(data.batch['rm_scores'], dim=-1)
    else:
        step_rewards = data.non_tensor_batch['rewards'].astype(np.float32)
...
def compute_advantage(data: DataProto, adv_estimator, gamma=1.0, lam=1.0, num_repeat=1, multi_turn=False, norm_adv_by_std_in_grpo=True, step_advantage_w=1.0, gigpo_mode="mean_std_norm", gigpo_enable_similarity=False, gigpo_similarity_thresh=0.95, use_process_only=False, **kwargs):
    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        data.batch['advantages'] = advantages
        data.batch['returns'] = returns
    elif adv_estimator == AdvantageEstimator.ISTAR_RLOO:
        advantages, returns = core_istar.compute_istar_rloo_advantage(
            data=data,
            step_advantage_w=step_advantage_w,
            use_process_only=use_process_only,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.ISTAR_GRPO:
        advantages, returns = core_istar.compute_istar_grpo_advantage(
            data=data,
            step_advantage_w=step_advantage_w,
            mode=gigpo_mode,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.ISTAR_REINFORCE_PLUS_BASELINE:
        advantages, returns = core_istar.compute_istar_reinforce_plus_baseline_advantage(
            data=data,
            step_advantage_w=step_advantage_w,
        )
        data.batch["advantages"] = advantages
        data.batch["returns"] = returns
    elif adv_estimator == AdvantageEstimator.PROCESS_OUTCOME_COMBINED:
        advantages, returns = core_istar.process_outcome_combined_advantage(
            data=data,
            step_advantage_w=step_advantage_w,
        )
        data.batch["advantages"] = advantages
...
class RayPPOTrainer:
    ...
    def __init__(
        ...
        if self.config.algorithm.adv_estimator == AdvantageEstimator.GAE:
            ...
            AdvantageEstimator.ISTAR_RLOO,
            AdvantageEstimator.ISTAR_GRPO,
            AdvantageEstimator.ISTAR_REINFORCE_PLUS_BASELINE,
            AdvantageEstimator.PROCESS_OUTCOME_COMBINED,
        ]:
            ...
            for batch_dict in self.train_dataloader:
                ...
                with _timer("step", timing_raw):
                    ...
                    with _timer("reward", timing_raw):
                        ...
                        if self.use_rm:
                            use_istar_rm_update = self.config.algorithm.adv_estimator in [
                                AdvantageEstimator.ISTAR_RLOO,
                                AdvantageEstimator.ISTAR_GRPO,
                                AdvantageEstimator.ISTAR_REINFORCE_PLUS_BASELINE,
                                AdvantageEstimator.PROCESS_OUTCOME_COMBINED,
                            ...
                            if not use_istar_rm_update or update_style == "none":
                                reward_output = self.rm_wg.compute_rm_score(batch)
                            elif update_style == "after":
                                if self.config.reward_model.model.get("loss_type", "ce") == "eto":
                                ...
                                bc_td = DataProto.from_dict(
                                    tensors={
                                        ...
                                        .sum(dim=-1)
                                        .view(-1, self.config.reward_model.num_rollout)
                                        .unsqueeze(1)
                                        .expand(-1, self.config.reward_model.num_rollout, -1)
                                        .reshape(-1, self.config.reward_model.num_rollout),
                                ...
                                raise NotImplementedError(f"Unknown reward_model.model.update={update_style}")
