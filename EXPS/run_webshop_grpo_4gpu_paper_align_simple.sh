...
echo "[ENV] GRPO full target confirmed: H100 (sm_90), GPUs=$VISIBLE_GPU_COUNT"
cleanup_ray
...
bash <(
  ...
  trainer.experiment_name=grpo_qwen2.5_1.5b_4gpu_paper_align_full_e250 \
  ...
  "$@" \
  2>&1 | tee "logs/webshop_grpo_qwen25_15b_4gpu_paper_align_simple_$(date +%Y%m%d_%H%M%S).log"
