...
echo "[ENV] GRPO smoke target: GPUs=$CUDA_VISIBLE_DEVICES"
cleanup_ray
...
bash <(
  ...
  trainer.experiment_name=grpo_qwen2.5_1.5b_2gpu_smoke \
  ...
  "$@" \
  2>&1 | tee "logs/webshop_grpo_qwen25_15b_2gpu_smoke_$(date +%Y%m%d_%H%M%S).log"
