...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def preprocess_data(self, data: DataProto) -> DataProto:
        ...
        if hasattr(vllm_ps, "get_tensor_model_parallel_group"):
            ...
            # vLLM>=0.7 wraps the process group in a coordinator.
            if hasattr(group, "device_group"):
                group = group.device_group
        elif hasattr(vllm_ps, "get_tp_group"):
            # vLLM>=0.15 switched to get_tp_group().
            group = vllm_ps.get_tp_group()
            if hasattr(group, "device_group"):
                group = group.device_group
        else:
            raise AttributeError("No tensor parallel group getter found in vllm.distributed.parallel_state")
