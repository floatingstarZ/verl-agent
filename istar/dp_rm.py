...
from istar.rm_utils import (compute_ce_dpo_loss_rm, compute_detach_dpo_loss_rm,
                            compute_eto_loss_rm, compute_eto_loss_rm_and_step_weights)
...
__all__ = ["DataParallelISTARRewardModel"]
...
class DataParallelISTARRewardModel:
    ...
    def update_rm_eto(self, data: DataProto):
        """
        Update reward model using trajectory-level ETO training.

        We first compute detached trajectory preferences for the full batch,
        then replay microbatches and backpropagate the equivalent first-order
        gradient. This avoids retaining every microbatch graph simultaneously.
        """
        self.reward_module.train()
        metrics = {}
        ...
        with torch.no_grad():
            for mini_batch_data in dataloader:
                if self.config.use_dynamic_bsz:
                    ...
                    micro_batches = mini_batch_data.split(self.config.micro_batch_size_per_gpu)
                ...
                for micro_batch_data in micro_batches:
                    ...
                    rm_scores_lst.append(rm_score.detach())
                    q_lst.append(q.detach())

        rm_scores = torch.cat(rm_scores_lst, dim=0)
        q_detached = torch.cat(q_lst, dim=0)

        eto_loss, step_weights = compute_eto_loss_rm_and_step_weights(
            data=data,
            token_level_scores=q_detached,
            beta=beta,
            env_name=env,
        )
        append_to_dict(metrics, {"reward_model/eto_loss": eto_loss.detach().item()})
        ...
        for mini_batch_data in dataloader:
            ...
            for micro_batch_data in micro_batches:
                ...
                _, q = self._forward_micro_batch(micro_batch_data, prompt_length)
                step_q = (q * response_mask).sum(dim=1) * beta
                cur_weights = step_weights[weight_offset : weight_offset + step_q.shape[0]].to(step_q.device)
                weight_offset += step_q.shape[0]

                surrogate_loss = (step_q * cur_weights).sum()
        ...
        metrics.update(
            {
                "reward_model/reward": rm_scores.sum(dim=-1).mean().item(),
                "reward_model/raw_reward": q_detached.sum(dim=-1).mean().item(),
            }
        )
